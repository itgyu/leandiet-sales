...
export const useStores = () => {
  ...
  const handleFetchStores = async (customFilters = {}) => {
    try {
      ...
      if (!success) {
        showError('한의원 목록을 불러오는데 실패했습니다.');
      }
      return success;
    } catch (err) {
      console.error('Fetch stores error:', err);
      showError('한의원 목록 조회 중 오류가 발생했습니다.');
      return false;
    }
  };
  ...
  const handleSelectStore = async (storeId) => {
    try {
      ...
      if (!success) {
        showError('한의원 정보를 불러오는데 실패했습니다.');
      }
      return success;
    } catch (err) {
      console.error('Select store error:', err);
      showError('한의원 정보 조회 중 오류가 발생했습니다.');
      return false;
    }
  };
  ...
  const handleUpdateStore = async (storeId, updateData) => {
    try {
      ...
      if (success) {
        showSuccess('한의원 정보가 수정되었습니다.');
      } else {
        showError('한의원 정보 수정에 실패했습니다.');
      }
      return success;
    } catch (err) {
      console.error('Update store error:', err);
      showError('한의원 정보 수정 중 오류가 발생했습니다.');
      return false;
    }
  };
  ...
  const handleUpdateStoreStatus = async (storeId, newStatus) => {
    try {
      ...
      if (success) {
        ...
        showSuccess(`한의원 상태가 '${statusLabel}'로 변경되었습니다.`);
      } else {
        showError('한의원 상태 변경에 실패했습니다.');
      }
      return success;
    } catch (err) {
      console.error('Update store status error:', err);
      showError('한의원 상태 변경 중 오류가 발생했습니다.');
      return false;
    }
  };
  ...
  /**
   * 새 한의원 생성 (에러 처리 포함)
   * @param {Object} storeData - 한의원 데이터
  ...
  const handleCreateStore = async (storeData) => {
    try {
      ...
      if (store) {
        showSuccess('새 한의원이 등록되었습니다.');
        return store;
      } else {
        showError('한의원 등록에 실패했습니다.');
        return null;
      }
    } catch (err) {
   ...
   * 한의원 삭제 (확인 다이얼로그 포함)
   * @param {string} storeId - 한의원 ID
   * @param {string} storeName - 한의원명 (확인용)
   * @returns {Promise<boolean>}
   */
  const handleDeleteStore = (storeId, storeName) => {
    return new Promise((resolve) => {
      showConfirmDialog(
        `'${storeName}' 한의원을 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.`,
        async () => {
          try {
            const success = await deleteStore(storeId);
            if (success) {
              showSuccess('한의원이 삭제되었습니다.');
            } else {
              showError('한의원 삭제에 실패했습니다.');
            }
            resolve(success);
          } catch (err) {
            console.error('Delete store error:', err);
            showError('한의원 삭제 중 오류가 발생했습니다.');
            resolve(false);
          }
        },
        () => resolve(false)
  ...
  const handleBulkUpdate = async (updates) => {
    try {
      ...
      if (result) {
        showSuccess(`${result.successCount}개 한의원이 업데이트되었습니다.`);
        if (result.errorCount > 0) {
          showError(`${result.errorCount}개 한의원 업데이트에 실패했습니다.`);
        }
      } else {

...
const MainLayout = ({ children, searchTerm, setSearchTerm, showSearch = false }) => {
  ...
  // 한의원 목록 로드 (최초 1회) - 검색용으로 전체 한의원 조회
  useEffect(() => {
    const loadStores = async () => {
      try {
        // all=true: GENERAL 유저도 전체 한의원 검색 가능
        ...
        console.error('한의원 목록 로드 실패:', err);
      }
    };
    loadStores();
  }, []);
          ...
          >
            <div style={{ position: 'relative' }}>
              ...
              <input
                ref={searchInputRef}
                type="text"
                placeholder="Seq / 한의원명 / 전화번호 / 주소 / POS 검색"
            ...
            item.hasDropdown ? (
              <div key={item.name} className="dropdown-container" style={{ position: 'relative' }}>
                <button
                  ...
                  style={{
                    ...
                    color: openDropdown === item.name || item.children?.some(child => location.pathname === child.path) ? '#8b5cf6' : '#374151',
                    border: 'none',
                    borderRadius: '8px',
                    fontSize: '14px',
                    fontWeight: '500',
                    ...
                    {item.children?.filter(child => !child.adminOnly || user?.role === 'ADMIN').map(child => (
                      <button
                        ...
                        style={{
                          ...
                          display: 'block',
                          padding: '10px 16px',
                          color: location.pathname === child.path ? '#8b5cf6' : '#374151',
              ...
              <button
                ...
                style={{
                  ...
                  color: location.pathname === item.path ? '#8b5cf6' : '#374151',
                  border: 'none',
                  borderRadius: '8px',
                  fontSize: '14px',
                  fontWeight: '500',

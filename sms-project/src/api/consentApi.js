...
export const createConsentLink = async (storeId) => {
  if (!storeId) {
    throw new Error('한의원 ID가 필요합니다.');
  }

  // 한의원 ID 기반으로 고유 링크 생성
...
/**
 * 한의원 정보 조회 (공개 API - JWT 인증 불필요)
 * @param {string} storeId - 한의원 ID (URL에서 받은 storeId 파라미터)
 * @returns {Promise<Object>} 한의원 정보 및 폼 필드
 */
export const getConsentForm = async (storeId) => {
  ...
  if (!result.success) {
    ...
    throw new Error(result.error || '한의원 정보 조회에 실패했습니다.');
  }
...
/**
 * 한의원의 동의서 응답 목록 조회 (관리자 API - JWT 인증 필요)
 * @param {string} storeId - 한의원 ID

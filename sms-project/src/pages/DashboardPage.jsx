...
// 담당자별 색상 (보라색 계열)
const MANAGER_COLORS = ["#8b5cf6", "#a78bfa", "#c4b5fd", "#ddd6fe", "#ede9fe"];

// 관리자 이메일 (필터링용)
const ADMIN_EMAIL = 'admin@leandiet.co.kr';
...
const DashboardPage = () => {
  ...
  const fetchStoreUsageByDate = useCallback(async () => {
    try {
      ...
      console.error('한의원 이용현황 조회 실패:', err);
    } finally {
      setLoading(false);
    }
    ...
    for (let i = 0; i < stores.length; i += CHUNK_SIZE) {
      ...
      const chunkPromises = chunk.map(async (store) => {
        try {
          ...
          // 개별 한의원 로그 조회 실패는 무시 (다른 한의원은 계속 처리)
        }
      });
  ...
  const funnelData = useMemo(() => {
    ...
    return [
      { name: '가입', value: overallStats.funnel.registered || 0, fill: '#8b5cf6' },
      { name: '설치', value: overallStats.funnel.install_completed || 0, fill: '#a78bfa' },
      { name: '이용', value: overallStats.funnel.active || 0, fill: '#FFA500' }
    ];
  }, [overallStats]);

  // 한의원 이용 현황 - 정렬된 한의원 리스트 (중복 계산 방지)
  ...
  const COLORS = ['#8b5cf6', '#a78bfa', '#FFA500', '#FFD700', '#32CD32', '#4169E1', '#9370DB', '#DC143C'];
              ...
              <div style={{
                ...
                <div
                  ...
                  style={{
                    padding: '16px',
                    borderRadius: '8px',
                    border: selectedInstallCategory === 'active' ? '2px solid #8b5cf6' : '1px solid #e5e7eb',
                ...
                <div
                  ...
                  style={{
                    padding: '16px',
                    borderRadius: '8px',
                    border: selectedInstallCategory === 'inactive' ? '2px solid #8b5cf6' : '1px solid #e5e7eb',
                ...
                <div
                  ...
                  style={{
                    padding: '16px',
                    borderRadius: '8px',
                    border: selectedInstallCategory === 'repair' ? '2px solid #8b5cf6' : '1px solid #e5e7eb',
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead>
                        <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
                          ...
                          <th style={{ width: '25%', padding: '8px', textAlign: 'left', fontSize: '12px', fontWeight: '600', color: '#6b7280' }}>한의원명</th>
                ...
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  ...
                  <button
                    ...
                    style={{
                      width: '36px',
                      height: '36px',
                      backgroundColor: chatInput.trim() && !chatLoading ? '#8b5cf6' : '#e5e7eb',
                      color: 'white',
                      border: 'none',
                      borderRadius: '8px',
                      display: 'flex',

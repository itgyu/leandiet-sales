...
const OrderUploadPage = () => {
  ...
  const handleFileSelect = async (files) => {
    ...
    try {
      ...
      // Step 4: DB 한의원 목록 가져오기
      setProcessingMessage('🏪 한의원 데이터베이스를 조회하고 있습니다...');
      await new Promise(resolve => setTimeout(resolve, 500));
      const fetchedStores = await fetchStores();
      setProcessingProgress(60);
      
      if (!fetchedStores || fetchedStores.length === 0) {
        showError('한의원 목록이 비어있습니다.');
        clearInterval(progressInterval);
        setIsProcessing(false);
        setProcessingProgress(0);
        setProcessingMessage('');
      ...
      // Step 5: 고유 한의원명 추출
      setProcessingMessage('🔍 고유한 한의원명을 추출하고 있습니다...');
      await new Promise(resolve => setTimeout(resolve, 500));
      const uniqueStoreNames = [...new Set(filtered.map(row => row['한의원명']))];
      setProcessingProgress(70);
      
      // Step 6: AI 매핑
      setProcessingMessage('🤖 AI가 유사한 한의원명을 매핑하고 있습니다...');
      await new Promise(resolve => setTimeout(resolve, 500));
  ...
  const fetchStores = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        throw new Error(response.error || '한의원 목록을 가져올 수 없습니다.');
      }
    } catch (err) {
      console.error('한의원 목록 조회 실패 - Full error:', err);
      console.error('Error message:', err.message);
      console.error('Error stack:', err.stack);
      showError('한의원 목록 조회에 실패했습니다.');
...
- none: No matching store exists`;
  ...
  const requestMapping = async (storeNames, dbStoresList) => {
    try {
      ...
      if (uncachedNames.length > 0) {
        ...
        setProcessingMessage(`🤖 ${uncachedNames.length}개 한의원 AI 매핑 시작...`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const ordersToSave = filteredOrders
        .map(order => {
          const seq = mappingMap.get(order['한의원명']);
          return {
            order_id: order['주문번호'],
            seq: seq === 'none' ? null : seq,  // 'none'인 경우 null로 저장
            store_name_csv: order['한의원명'],
  ...
  const downloadSampleCSV = () => {
    const sampleData = `주문번호,한의원명,회원명,주문시간,결제모듈,결제상태,쿠폰할인금액,결제금액,결제시간
                ...
                <React.Fragment key={index}>
                  <div style={{
                    ...
                    <div style={{
                      ...
                      height: '32px',
                      borderRadius: '50%',
                      backgroundColor: currentStep >= index ? '#8b5cf6' : '#e5e7eb',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center'
                    }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <button
                ...
                style={{
                  padding: '10px 24px',
                  backgroundColor: isSaving ? '#9ca3af' : '#8b5cf6',
                  color: 'white',
                  border: 'none',
                  borderRadius: '8px',
                  fontSize: '14px',

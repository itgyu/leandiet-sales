...
const StoreListPage = () => {
  ...
  const handleExport = () => {
    ...
    const now = new Date();
    const fileName = `한의원목록_${now.getFullYear()}${String(now.getMonth()+1).padStart(2,'0')}${String(now.getDate()).padStart(2,'0')}.csv`;
  ...
  const handleAddStore = useCallback(async (e) => {
    ...
    try {
      ...
      if (result) {
        ...
        alert('한의원 생성에 실패했습니다.');
      }
    } catch (err) {
      console.error('한의원 생성 실패:', err);
      alert('한의원 생성에 실패했습니다: ' + err.message);
    } finally {
  ...
  const parseCSV = (text) => {
    ...
    return dataLines.map((line, index) => {
      ...
      if (columns.length < 3) {
        throw new Error(`${index + 2}번째 줄: 필수 컬럼이 부족합니다. (Seq, 한의원명, 전화번호 필수)`);
      }

      // Seq 검증
      if (!columns[0]) {
        throw new Error(`${index + 2}번째 줄: Seq(한의원 고유번호)는 필수입니다.`);
      }

      // 한의원명 검증
      if (!columns[1]) {
        throw new Error(`${index + 2}번째 줄: 한의원명은 필수입니다.`);
      }
      ...
      return {
        seq: columns[0],        // 한의원 고유번호 (필수)
        name: columns[1],       // 한의원명 (필수)
  ...
  const createSingleStore = async (storeData) => {
    try {
      ...
      if (result) {
        ...
        return { success: false, store: storeData, error: '한의원 생성 실패 - 결과 없음' };
      }
    } catch (error) {
      console.error('한의원 생성 실패:', error.message, storeData);
  ...
  const handleCSVUpload = async () => {
    ...
    try {
      ...
      if (stores.length === 0) {
        throw new Error('업로드할 한의원 데이터가 없습니다.');
      }
      ...
      for (let i = 0; i < stores.length; i++) {
        ...
        if (result.success) {
          ...
          errors.push(`${i + 1}번째 한의원 [${stores[i].seq}] ${stores[i].name}: ${result.error}`);
        }
        ...
        <div className="hidden md:block" style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
            <h3 style={{ 
              ...
              <span>한의원 목록 ({filteredStores.length}개)</span>
            </h3>
            ...
            <form onSubmit={handleAddStore}>
              ...
              <div style={{ marginBottom: '24px' }}>
                <label style={{ 
                  ...
                  한의원 고유번호 <span style={{ color: '#ef4444' }}>*</span>
                </label>
                <input
                  type="text"
                  value={newStore.seq}
              ...
              <div style={{ marginBottom: '24px' }}>
                <label style={{ 
                  ...
                  한의원명 <span style={{ color: '#ef4444' }}>*</span>
                </label>
                <input
                  type="text"
                  value={newStore.name}
              ...
              <div style={{ display: 'flex', gap: '12px' }}>
                ...
                <button
                  ...
                  style={{
                    flex: 1,
                    padding: '14px 20px',
                    backgroundColor: isSubmitting ? '#ddd6fe' : '#8b5cf6',
                    color: 'white',
                    border: 'none',
                    borderRadius: '12px',
                    fontSize: '15px',
                  ...
                  onMouseOut={(e) => {
                    if (!isSubmitting) e.target.style.backgroundColor = '#8b5cf6';
                  }}
                >
                  {isSubmitting && (
                    <div style={{ 
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                  ...
                  <div>
                    ...
                    <ul style={{
                      ...
                      <li>첫 번째 행: Seq, 한의원명, 전화번호, 담당자이메일</li>
                      <li>두 번째 행부터: 실제 한의원 데이터 입력</li>
                      <li>필수 항목: Seq(한의원 고유번호), 한의원명, 전화번호</li>
              ...
              {uploadStatus === 'idle' && (
                <button
                  ...
                  style={{
                    flex: 1,
                    padding: '14px 20px',
                    backgroundColor: !selectedFile ? '#d1d5db' : '#8b5cf6',
                    color: 'white',
                    border: 'none',
                    borderRadius: '12px',
                    fontSize: '15px',
                  ...
                  onMouseOver={(e) => {
                    if (selectedFile) e.target.style.backgroundColor = '#6d28d9';
                  }}
                  onMouseOut={(e) => {
                    if (selectedFile) e.target.style.backgroundColor = '#8b5cf6';
                  }}
                >
                  CSV 업로드 시작
                </button>
